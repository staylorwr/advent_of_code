  ...
  defp parse(input) do
    ...
    |> Enum.map(fn group ->
      surveys =
        group
        |> String.split("\n", trim: true)
        |> Enum.map(fn x -> x |> String.graphemes() |> MapSet.new() end)
      ...
      every =
        ...
        |> Enum.filter(fn letter ->
          Enum.all?(surveys, fn survey ->
            MapSet.member?(survey, letter)
          end)
        end)
        |> MapSet.new()
        |> MapSet.size()

      %{surveys: surveys, total: MapSet.size(collective), every: every}
    end)
  end
end
