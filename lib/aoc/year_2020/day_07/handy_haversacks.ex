defmodule Aoc.Year2020.Day07.Graph do
  defstruct [:g, :bags]

  def new(bags) do
    graph = Enum.reduce(bags, Graph.new(), &add_bag(&2, &1))
    %__MODULE__{g: graph, bags: bags}
  end

  def add_bag(graph, {bag, rules}) do
    Enum.reduce(rules, graph, fn {weight, child}, g ->
      Graph.add_edge(g, bag, child, weight: weight)
    end)
  end
end
...
defmodule Aoc.Year2020.Day07.HandyHaversacks do
  ...
  *How many individual bags are required inside your single `shiny gold` bag?*
  ...
  """
  alias Aoc.Year2020.Day07.Graph, as: BagGraph
  ...
  def part_1(input) do
    ...
    graph.bags
    |> Enum.filter(fn {selected, _} -> selected != @bag end)
    |> Enum.map(fn {selected, _} ->
      Graph.get_shortest_path(graph.g, selected, @bag)
    end)
    |> Enum.filter(& &1)
    |> Enum.count()
  end
  ...
  def part_2(input) do
    ...
    do_part_2(graph.g, @bag, 1)
  end

  def do_part_2(graph, v, factor) do
    Graph.out_edges(graph, v)
    |> Enum.reduce(0, fn %Graph.Edge{v2: v2, weight: w}, sum ->
      sum + w * factor + do_part_2(graph, v2, factor * w)
    end)
  end
  ...
  defp do_parse_line([bag, contents]), do: {bag, parse_bag_contents(contents)}

  defp parse_bag_contents(contents) do
    ~r/(\d+) (\w+ \w+)/
    |> Regex.scan(contents)
    |> Enum.map(fn [_, quantity, bag] ->
      {String.to_integer(quantity), bag}
    end)
  end
end
