defmodule Aoc.Year2018.Day12.SubterraneanSustainability do
  @moduledoc """
  ...
  - A note like `.##.# => #` means that a pot has a plant in a given generation if,
    in the previous generation, there were plants in that pot, the one immediately to
    the left, and the one two pots to the right, but not in the ones immediately to the right
    and two to the left.
  ...
  *After `20` generations, what is the sum of the numbers of all pots which
  ...
  def new(input, options) do
    ...
    %__MODULE__{generation: generation, view: view, patterns: patterns}
    |> build_pots(input)
    |> update_total
  end
  ...
  def parse_patterns(input) do
    input
  end

  def build_pots(state, input) do
    state
  end

  def update_total(state), do: state
end
