defmodule Aoc.Year2018.Day09.MarbleMania do
  @moduledoc """
  ## --- Day 9: Marble Mania ---
  ...
  *What is the winning Elf's score?*
  ...
  @doc """
  Iterate through a game of marbles, tracking the scores
  of the players until the last marble count is reached.
  ...
  def winning_score(player_count, last_marble) do
    player_count
    |> new(last_marble)
    |> play_game(player_count, 1)
    |> Map.get(:scores)
    |> Map.values()
    |> Enum.max()
  end

  defp play_game(%{next_marble: next, last_marble: last} = game, _player, _current)
       when next > last,
       do: game

  defp play_game(game, player_count, current_player) do
    next_player = rem(current_player + 1, player_count)
  ...
  def player_turn(game, _player) do
    marbles =
      ...
      |> ZipList.next()
      |> ZipList.next()
      |> ZipList.insert(game.next_marble)

    %{game | marbles: marbles, next_marble: game.next_marble + 1}
  end
