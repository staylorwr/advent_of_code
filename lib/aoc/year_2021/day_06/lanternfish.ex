defmodule Aoc.Year2021.Day06.Lanternfish do
  @moduledoc """
  ## --- Day 6: Lanternfish ---
  ...
  `3,4,3,1,2`This list means that the first fish has an internal timer of `3`, the second
  ...
  @doc """
  Find a way to simulate lanternfish.  Given an initial state and number of
  days to simulate, return the total count of lanternfish on that day.

  ## Examples


      iex> part_1("3,4,3,1,2", 18)
      26
      iex> part_1("3,4,3,1,2", 80)
      5934

  """
  def part_1(input, days) do
    initial_state = parse(input)

    1..days
    |> Enum.reduce(initial_state, fn _x, acc ->
      cycle(acc)
    end)
    |> Map.values()
    |> Enum.sum()
  ...
  After  7 days: 3,4,3,1,2,3,4,5,5,6
      ...
      iex> cycle(%{1 => 1, 2 => 1, 3 => 2, 4 => 1})
      %{0 => 1, 1 => 1, 2 => 2, 3 => 1}
      iex> cycle(%{0 => 1, 1 => 1, 2 => 2, 3 => 1})
      %{0 => 1, 1 => 2, 2 => 1, 6 => 1, 8 => 1}
  """
  def cycle(timers) do
    {pregnancies, others} = Map.pop(timers, 0, 0)
