defmodule Aoc.Year2022.Day03.RucksackReorganization do
  ...
  *priority*:
  ...
  the sum of the priorities of those item
  ...
  def part_2(input) do
    input
    |> parse()
    |> Enum.map(fn {first, second} -> MapSet.union(first, second) end)
    |> Enum.chunk_every(3)
  ...
  defp parse(input) do
    ...
    |> String.split("\n", trim: true)
    |> Enum.map(fn pack ->
      all_items = String.codepoints(pack)

      total_items = length(all_items)
      half_of_items = div(total_items, 2)

      {first, second} = Enum.split(all_items, half_of_items)
      {MapSet.new(first), MapSet.new(second)}
    end)
  end

  defp item_to_priority(item) do
    characters = String.codepoints(@lowercase <> @uppercase)
    priorities = Enum.with_index(characters, 1)
    Enum.into(priorities, %{})[item]
  end
end
