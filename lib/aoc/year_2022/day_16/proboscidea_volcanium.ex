defmodule Aoc.Year2022.Day16.ProboscideaVolcanium do
  alias __MODULE__.{Parser, Logic}
  ...
  defmodule Logic do
    def solve_1(graph) do
      graph
      |> find_paths(30, 0, fn _, vent, prev -> prev + vent end)
      |> Enum.max()
    end
    ...
    def find_paths(graph, time, init, res_fun) do
      valves = valves_with_rate(graph)
      merged = Map.merge(rates(graph), distances(graph))
      find_paths(:AA, time, valves, init, merged, res_fun)
    end
