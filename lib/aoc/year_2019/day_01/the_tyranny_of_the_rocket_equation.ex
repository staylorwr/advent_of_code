defmodule Aoc.Year2019.Day01.TheTyrannyoftheRocketEquation do
  ...
  *What is the sum of the fuel requirements* for all of the modules on your
  ...
  @doc """
  Calculate the fuel required per module

  Examples:

      fuel_for_dry_mass(12) == 2
  """
  def fuel_for_dry_mass(val), do: floor(val / 3) - 2

  @doc """
  Calculate the total mass of fuel required to lift
  the payload and the fuel itself
  """
  def fuel_for_wet_mass(input), do: fuel_for_wet_mass(input, 0)

  def fuel_for_wet_mass(remaining, total) when remaining <= 0, do: total

  def fuel_for_wet_mass(remaining, total) do
    next = fuel_for_dry_mass(remaining)
    if next >= 0, do: fuel_for_wet_mass(next, total + next), else: fuel_for_wet_mass(0, total)
  end
end
